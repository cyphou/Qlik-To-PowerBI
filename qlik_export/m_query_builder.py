...
# ═══════════════════════════════════════════════════════════════════
# Auto-chaining convenience wrappers
#
# These functions produce (step_name, step_code) tuples that use a
# placeholder for the previous step.  inject_m_steps() resolves the
# placeholder to the actual last step name automatically.
# ═══════════════════════════════════════════════════════════════════

_PREV_PLACEHOLDER = "__PREV__"


def m_transform_rename(mapping: Dict[str, str]) -> Tuple[str, str]:
    """Auto-chaining rename columns (no explicit prev required)."""
    return rename_columns(_PREV_PLACEHOLDER, mapping)


def m_transform_remove_columns(columns: List[str]) -> Tuple[str, str]:
    """Auto-chaining remove columns (no explicit prev required)."""
    return remove_columns(_PREV_PLACEHOLDER, columns)


def m_transform_filter_values(column: str, values: List[str]) -> Tuple[str, str]:
    """Auto-chaining filter values (no explicit prev required)."""
    return filter_values(_PREV_PLACEHOLDER, column, values)


def m_transform_filter_nulls(column: str, keep_nulls: bool = False) -> Tuple[str, str]:
    """Auto-chaining filter nulls (no explicit prev required)."""
    return filter_nulls(_PREV_PLACEHOLDER, column, keep_nulls)


def m_transform_add_column(name: str, expression: str,
                           m_type: str = "type text") -> Tuple[str, str]:
    """Auto-chaining add column (no explicit prev required).

    Unlike ``add_custom_column``, *expression* is used verbatim (the caller
    must include ``each`` if desired).  Step names are unique, derived from
    the column name.
    """
    safe_name = re.sub(r'[^A-Za-z0-9_]', '_', name)
    step_name = f"Added_{safe_name}"
    return (step_name,
            f'    {step_name} = Table.AddColumn({_PREV_PLACEHOLDER}, {_m_col(name)}, '
            f'{expression}, {m_type})')
...
def inject_m_steps(m_query: str, steps: List[Tuple[str, str]]) -> str:
    ...
    # Determine last step name for auto-chaining (_PREV_PLACEHOLDER support)
    prev_step_name = "Source"
    for line in reversed(pre_in):
        stripped = line.strip().rstrip(',')
        if "=" in stripped and not stripped.startswith("//"):
            prev_step_name = stripped.split("=")[0].strip()
            break

    # Add new steps, resolving _PREV_PLACEHOLDER to actual previous step
    for i, (step_name, step_code) in enumerate(steps):
        resolved_code = step_code.replace(_PREV_PLACEHOLDER, prev_step_name)
        if i < len(steps) - 1:
            pre_in.append(resolved_code + ",")
        else:
            pre_in.append(resolved_code)
