param(
    [string]$Repo = "cyphou/Qlik-To-PowerBI"
)
...
$issues = @(
    @{
        title = "[P0] Openability guard: add root-cause taxonomy and healer effectiveness metrics"
        body = @"
## Objective
...
"@
    ...
    @{
        title = "[P0] Extend deterministic DAX remediation pack with guarded rewrite policies"
        body = @"
## Objective
...
"@
    ...
    @{
        title = "[P0] Extend deterministic Power Query M remediation coverage"
        body = @"
## Objective
...
"@
    ...
    @{
        title = "[P1] Fidelity hardening: many-to-many relationship synthesis and visual binding recovery"
        body = @"
## Objective
...
"@
    ...
    @{
        title = "[P1] Release gate workflow: enforce openability + qa + schema + cross-validate"
        body = @"
## Objective
...
"@
    ...
    @{
        title = "[P1] Performance and scale: profile/optimize openability and autoheal pipeline"
        body = @"
## Objective
...
foreach ($issue in $issues) {
    gh issue create --repo $Repo --title $issue.title --body $issue.body | Out-Host
}

Write-Output "Created $($issues.Count) issues on $Repo"
